...
case 'buscarpareja':
  if (user.marriage) {
    return conn.reply(m.chat, `💞 Ya estás en una relación con @${user.marriage.split('@')[0]}.`, m, { mentions: [user.marriage] })
  }

  // Obtener usuarios disponibles (que no estén casados)
  let availableUsers = Object.entries(global.db.data.users)
    .filter(([jid, data]) => data && !data.marriage && jid !== m.sender)
    .map(([jid]) => jid)

  if (availableUsers.length == 0) {
    return conn.reply(m.chat, `😢 No hay personas solteras disponibles en este momento... Inténtalo más tarde.`, m)
  }

  // Elegir aleatoriamente una pareja
  let partnerJid = availableUsers[Math.floor(Math.random() * availableUsers.length)]
  user.marriage = partnerJid
  global.db.data.users[partnerJid].marriage = m.sender
